...
current_milli_time = lambda: int(round(time.time() * 1000)) #Todo: change
...
def DQN(env_name, hidden_sizes=[32], lr=1e-2, num_epochs=2000,
        ...
        batch_size=64, update_freq=4, frames_num=2, min_buffer_size=5000,
        test_frequency=20, start_explor=1, end_explor=0.1, explor_steps=100000):
    # Create the environment both for train and test
    env = make_env(env_name, frames_num=frames_num, skip_frames=True,
                   noop_num=20)
    env_test = make_env(env_name, frames_num=frames_num, skip_frames=True,
                        noop_num=20)
    # Add a monitor to the test env to store the videos
    env_test = gym.wrappers.Monitor(env_test,
                                    "VIDEOS/TEST_VIDEOS" + env_name + str(
                                        current_milli_time()), force=True,
    ...
    obs_ph = tf.placeholder(shape=(None, obs_dim[0], obs_dim[1], obs_dim[2]),
    ...
    update_target = [train_vars[i].assign(train_vars[i + len(target_vars)]) for
                     i in range(len(train_vars) - len(target_vars))]
    ...
    def agent_op(o):
        # Forward pass to obtain the Q-values
        # Scale the frames
        o = scale_frames(o)
    ...
    hyp_str = "-lr_{}-upTN_{}-upF_{}-frms_{}".format(lr, update_target_net,
    ...
    for ep in range(num_epochs):
        ...
        while not done:
            ...
            if (len(buffer) > min_buffer_size) and (
                ...
                # run the session to update the target network and get the MSE
        ...
        if ep % test_frequency == 0:
            ...
            # Run the test stats
            test_summary = sess.run(reward_summary,
                                    feed_dict={mr_v: np.mean(test_rw)})
            ...
            print(
                ...
                (ep, np.mean(batch_rew), eps, step_count, np.mean(test_rw),
                 np.std(test_rw), ep_sec_time,
...
if __name__ == '__main__':
    DQN('PongNoFrameskip-v4', hidden_sizes=[128], lr=2e-4, buffer_size=100000,
